 ...
 * In background (interactive = false), if the token is expired or missing,
 * it NEVER triggers an intrusive Google Account chooser popup on mobile!
 */
export async function getOrRefreshGoogleHealthToken(interactive = false, hintEmail = null) {
    ...
    // Se non è collegato o se siamo in background e il token è scaduto, NON mostrare popup!
    if (!interactive) {
        if (!isGoogleHealthConnected()) {
            throw new Error('Pixel Watch / Google Health non collegato.');
        }
        // Il token è scaduto: in background non blocchiamo l'utente con popup di login
        throw new Error('Token Pixel Watch scaduto. Ricollega dal Profilo.');
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const requestOptions = { prompt: 'consent' };
            if (email) {
 ...
 */
export async function resyncDayWithGoogleHealth(date, meals) {
    const token = await getOrRefreshGoogleHealthToken();
