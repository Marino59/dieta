...
export async function getDailyCoachAdvice(profile, caloriesConsumed, weights = []) {
    try {
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        ...
        const profileContext = JSON.stringify({
            ...
            currentTime: new Date().toLocaleTimeString('it-IT') // Added entropy
        });
        ...
        1. Fornisci un consiglio motivazionale molto breve ma potente (max 15-20 parole). 
           ...
           Se disponi dello storico peso, commenta l'andamento (es. "Grande progresso con i -X kg!" o "Stiamo tenendo botto, non mollare il peso!") o incoraggia a pesarsi se i dati sono vecchi o assenti.
